...
jQuery('#jqReportcol'+col).jqGrid({
    ...
    datatype: "local",
    colNames:['Срок действия РУ','Кол-во','Перейти'],
    colModel:[
        {name:'col'+col,index:'col'+col, align:'center', width:250, formatter:"date",
            searchoptions: { sopt:['rn'], dataInit: function(el) {
                    $(el).datepick($.extend({ showTrigger: '#calImg',
                        rangeSelect: true, 
                onClose: function(dates) { 
                    idGrid="#jqReportcol4_data";                    
                    $(idGrid)[0].triggerToolbar(); 
                }
